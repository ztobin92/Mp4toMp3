...
final class EditViewController: BaseViewController<EditViewModel> {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if #available(iOS 16, *) { viewModel.router.presentEditBottomSheet(editVM: viewModel) } else {}
        }
    }
    ...
    func didTapExport() {
        ...
        viewModel.didTapExport { [weak self] urls, allParams in
            ...
            if urls.count == 1, allParams.count == 1 {
                if let vc = AppRouter.shared.topViewController() as? EditBottomSheetViewController, let nav = navigationController, let urlOpt = urls.first, let urlOpt {
                    vc.viewModel.router.presentExport(parameters: .init(rootNav: nav, assetURL: urlOpt, outputParameters: allParams.first!))
                }
            } else {
                if let vc = AppRouter.shared.topViewController() as? EditBottomSheetViewController, let nav = navigationController {
